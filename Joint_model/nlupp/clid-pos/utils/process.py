...
def multilabel2one_hot(labels, nums):#將多標籤（multilabel）格式的數據轉換成 one-hot 編碼格式
    res = [0.] * nums
    if len(labels) == 0:
        return res
...
class Processor(object):
    ...
    def train(self):
        ...
        best_epoch = 0
        no_improve = 0
        dataloader = self.__dataset.batch_delivery_tp('train')
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            for text_batch, slot_batch, intent_batch in tqdm(dataloader, ncols=50):
                padded_text, [sorted_slot, sorted_intent], seq_lens = self.__dataset.add_padding(
                    text_batch, [(slot_batch, True), (intent_batch, True)])#在做clid時要使用True

                window_seq_lens = []
                #這個迴圈是為了解決遇到window_size>對話字數的情況，也要切成chunk，但是要稍微調整一下。
                for i, seq_len in enumerate(seq_lens) :
                    if self.args.window_size<seq_len:#如果遇到對話字數>window_size
                        window_seq_lens.append((seq_len - self.args.window_size + 1)) 
                    else:#如果遇到window_size>對話字數，也切成chunk，等於1。
                        window_seq_lens.append(1) 

                text_var = torch.LongTensor(padded_text)             
                slot_var = torch.LongTensor(sorted_slot)#LongTensor處理整數張量
                #intent_var = torch.Tensor(sorted_intent)#Tensor處理整數張量

                #將intent_golden_labl做成chunk_intent_golden_labl，使用window_size去切割。
                start_chunk = (self.args.window_size + 1) // 2 - 1 #找出第一個chunk的位置。
                chunk_labels_all = []# 為每一行的標籤創建一個新的chunk_labels列表
                for row in sorted_intent:
                    chunk_labels = []
                    chunk_num = len(row) - self.args.window_size + 1
                    for i in range(chunk_num):
                        chunk_labels.append(row[start_chunk + i])
                    chunk_labels_all.append(chunk_labels)

                intent_var = torch.tensor(chunk_labels_all, dtype=torch.long)
                ...
                if random_slot < self.__dataset.slot_forcing_rate:
                    slot_out, intent_out = self.__model(text_var, seq_lens, window_seq_lens, self.args.window_size, forced_slot=slot_var)#ModelManager的forward
                else:
                    slot_out, intent_out = self.__model(text_var, seq_lens, window_seq_lens, self.args.window_size)

                slot_var = torch.cat([slot_var[i][:seq_lens[i]] for i in range(0, len(seq_lens))], dim=0)#獲取每一個對話seq_len和做成1維張量。
                slot_loss = self.__criterion(slot_out, slot_var)#預測的slot機率和slot_golden_label。    

                intent_var = torch.cat([intent_var[i][:seq_lens[i] - self.args.window_size + 1] for i in range(0, len(seq_lens))], dim=0)#處理intent_golden_labl。
                #print(intent_var.size(), intent_var, intent_out.size(), intent_out)
                #input()
    ...
    def estimate(self, if_dev, args, test_batch=100):
        ...
        mistake_dir = os.path.join(args.save_dir, "error")
        os.makedirs(mistake_dir, exist_ok=True)

        # Write those sample both have intent and slot errors.
        with open(os.path.join(mistake_dir, 'error.txt'), 'w', encoding="utf8") as fw:
    ...
    def prediction(model, dataset, mode, batch_size, args):
        ...
        all_token = []#做slot filling用不影響意圖偵測
        for text_batch, slot_batch, intent_batch in tqdm(dataloader, ncols=50):
            ...
            #這個迴圈是為了解決遇到window_size>對話字數的情況，也要切成chunk，但是要稍微調整一下。
            for i, seq_len in enumerate(seq_lens) :
                if args.window_size<seq_len:#如果遇到對話字數>window_size
                    window_seq_lens.append((seq_len - args.window_size + 1)) 
                else:#如果遇到window_size>對話字數，也切成chunk，等於1。
                    window_seq_lens.append(1) 

            real_slot.extend(sorted_slot)
            all_token.extend([pt[:seq_lens[idx]] for idx, pt in enumerate(padded_text)])
            for intents in list(Evaluator.expand_list(sorted_intent)):#使用'#'將意圖標籤分開
                if '#' in intents:
                    real_intent.append(intents.split('#'))
                else:
                    real_intent.append([intents])

            digit_text = dataset.word_alphabet.get_index(padded_text)#這是甚麼?

            var_text = torch.LongTensor(digit_text)
            max_len = np.max(seq_lens)
            if args.gpu:
                var_text = var_text.cuda()
            slot_idx, intent_idx = model(var_text, seq_lens, window_seq_lens, args.window_size, n_predicts=1)
        ...
        [p_intent.sort() for p_intent in pred_intent]#進行排列
        [r_intent.sort() for r_intent in real_intent]

        with open(os.path.join(args.save_dir, 'token.txt'), "w", encoding="utf8") as writer:#slot filling的結果儲存起來
            idx = 0
...
class Evaluator(object):
    ...
    def semantic_acc(pred_slot, real_slot, pred_intent, real_intent):
        """
        根据给定句子的完整预测结果计算准确度，包括槽位（slot）和意图（intent）。
        """
        total_count, correct_count = 0.0, 0.0
