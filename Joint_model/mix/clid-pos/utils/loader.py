...
class DatasetManager(object):
    ...
    def add_file(self, file_path, data_name, if_train_file):
        text, slot, intent = self.__read_file(file_path)#測試集和驗證集

        if if_train_file:
            print(f'原資料集:{len(text)}')
            if self.__args.aug_file_path != 'None':  # 確認是否有進行資料增強
                aug_text, aug_slot, aug_intent = self.__read_file(self.__args.aug_file_path)
        ...
        if if_train_file:#如果是訓練集的話
            ...
            #self.__digit_tp_intent_data[data_name] = tp_intent#intent_tp的index
    ...
    def batch_delivery_tp(self, data_name, batch_size=None, is_digital=True, shuffle=True):
        ...
        if is_digital:#訓練集拿文字的index
            ...
            tp_intent_data = []
            texts = self.__text_word_data[data_name]
            for i, words in enumerate(texts):  # 找每一個句子
                tp_intent_data.append([])  # 为每个句子初始化一个空列表
                count = 0  # 用來計算多意圖標籤用的
                # 检查intent[i]的类型
                if isinstance(intent[i], list):
                    # 如果intent[i]是一个嵌套列表，則取第一个元素
                    if len(intent[i]) == 1 and isinstance(intent[i][0], list):
                        intent[i] = intent[i][0]
                else:
                    # 如果intent[i]不是列表（例如是一个整数），則將其轉換为单元素列表
                    intent[i] = [intent[i]]

                tagged_words = nltk.pos_tag(words)
                for k, tagged_word in enumerate(tagged_words):
                    if tagged_word[1] == 'CC':  # 找到連接詞，標註TP，多意圖標籤加1。

                        tp_intent_data[i].append(len(self.intent_alphabet))#TP標籤放到最後一個
                        if len(intent[i]) > count + 1:  # 確保count不會超出intent[i]的範圍
                            count += 1
                    else:
                        tp_intent_data[i].append(intent[i][count])
                #print(len(words),words)
                #print(len(tp_intent_data[i]),tp_intent_data[i])
                #input()                
