    ...
    def train(self):
        ...
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            for text_batch, slot_batch, intent_batch in tqdm(dataloader, ncols=50):
                ...
                #print(seq_lens)
                sorted_intent_exp = []
                for item, num in zip(sorted_intent, window_seq_lens):#每一個句子的chunk數，每一個chunk都放多意圖標籤。
                ...
                sorted_intent = [multilabel2one_hot(intents, len(self.__dataset.intent_alphabet)) for intents in
                                 sorted_intent_exp] #將chunk裡的多意圖標籤，轉成one_hot encoding。            
                ...
                if random_slot < self.__dataset.slot_forcing_rate:
                    slot_out, intent_out = self.__model(text_var, seq_lens, window_seq_lens, self.args.window_size, forced_slot=slot_var)
                else:
                ...
                intent_out = torch.cat([intent_out[i][:window_seq_lens[i] ] for i in range(0, len(seq_lens))], dim=0)#處理intent_golden_labl。
