...
def write_file(file_path, texts, slots, intents):#寫檔
    ...
    with open(file_path, 'w') as file:# train 、 dev 、 test 。
        for text, slot, intent in zip(texts, slots, intents):
            tagged_text = '\n'.join([f"{word} {tag}" for word, tag in zip(text, slot)])
            file.write(f"{tagged_text}\n")
            file.write(f"{'#'.join(intent)}\n\n" if intent else "\n")

def read_file(file_path):#讀檔
    texts, slots, intents = [], [], []
    text, slot = [], []
...
def slot_sub_augment(texts, slots, intents, method_num):
    dictionary = slot_dictionary(texts, slots)
    new_text, new_slot, new_intent = [], [], []
