...
class Alphabet(object):
    ...
    # Alphabet 类中的 add_instance 方法的实现，用于向 Alphabet 中添加实例。
    def add_instance(self, instance, multi_intent=False):
        ...
        #如果 multi_intent 为 True，并且实例中包含 # 字符，方法将根据 # 分隔符将实例拆分为多个子实例，并将每个子实例分别添加到 Alphabet 中。
        #方法会统计每个实例的出现频率，以便影响未知实例的序列化。
        ...
        #如果實例尚未存在于 Alphabet 中，方法会将實例添加到 __index2instance 列表中，並在 __instance2index 字典中記錄实例和其對應的索引。
...
class DatasetManager(object):
    ...
    def quick_build(self):
        ...
        #使用 if_train_file=True 或 if_train_file=False 来指示是否是訓練文件。
