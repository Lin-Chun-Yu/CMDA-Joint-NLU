...
class Alphabet(object):
    ...
    # Alphabet 類中的 add_instance 方法的實現，用于向 Alphabet 中添加實例。
    def add_instance(self, instance, multi_intent=False):#instance是指文字、槽標籤、意圖標籤。
        ...
        """
        #方法首先檢查 instance 是否是一个列表或元组。如果是，它將递归调用自己来處理列表中的每个元素，将它们作为单独的实例添加到 Alphabet 中。
        #如果 instance 不是列表或元组，方法会檢查它是否是字符串類型。只有字符串类型的實例才会被添加到 Alphabet 中。
        ...
        #如果 multi_intent 为 True，并且實例中包含 # 字符，方法將根據 # 分隔符將實例拆分為多個子實例，並將每個子實例分别添加到Alphabet中。
        #方法會計算每个實力的出現的次數，以便影響未知實例的序列化。
        ...
        #如果實例尚未存在于 Alphabet 中，方法會將實例添加到 __index2instance 列表中，並在 __instance2index 字典中記錄實例和其對應的索引。
    ...
    #用于根據给定的索引返回 Alphabet 中對應的標籤名稱，slot和intent都有可能。
    ...
    #將內容(_dict.txt、_list.txt)儲存在Alphabet
...
#它是 torch.utils.data.Dataset 的子類，
#用于實現一个資料集对象，以便能夠通过 DataLoader 来批量加載數據。
...
class DatasetManager(object):
    ...
    def __init__(self, args):#初始化資料集管理器

        #self.__word_alphabet：用於儲存Vocabulary資訊。在建構时，它使用了兩個Boolean值 if_use_pad 和 if_use_unk 来指定是否使用特殊的填充符号（pad）和未知符号（unk）。
        #self.__slot_alphabet：用於儲存槽位（slot）資訊。同样，它也使用了兩個Boolean值參數来指定是否使用填充符号和未知符号。
        #self.__intent_alphabet：用於儲存意圖（intent）資訊，同樣也可以指定是否使用填充符號和未知符號。
        self.__word_alphabet = Alphabet('word', if_use_pad=True, if_use_unk=True)# class Alphabet
        self.__slot_alphabet = Alphabet('slot', if_use_pad=False, if_use_unk=False)# class Alphabet
        self.__intent_alphabet = Alphabet('intent', if_use_pad=False, if_use_unk=False)# class Alphabet
        ...
        self.__digit_tp_intent_data = {}#我加的，為了找tp。

        self.__args = args

    @property
    def test_sentence(self):
        return deepcopy(self.__text_word_data['test'])#獲取測試集中的句子
    ...
    @property
    def tp_index(self):
        return self.__intent_alphabet.get_index('TP') 
    ...
    def quick_build(self):
        ...
        #使用 if_train_file=True 或 if_train_file=False 来確認是否是訓練集。
    ...
    def get_dataset(self, data_name, is_digital):
        ...
        :param data_name: 是儲存的資料集的名稱。
        :param is_digital: 確定是否需要序列化資料。
        :return: 需要的資料集。
        """
    ...
    #用於將數據文件的内容添加到數據集對象中，並進行詞彙表的建構和數據的序列化。
    def add_file(self, file_path, data_name, if_train_file):

        if if_train_file:#如果是訓練集的話
            text, slot, intent, tp_intent = self.__read_tp_file(file_path)
            print(f'原資料集:{len(text)}')
            if self.__args.aug_file_path != 'None':  # 確認是否有進行資料增強
                aug_text, aug_slot, aug_intent, aug_tp_intent = self.__read_tp_file(self.__args.aug_file_path)
                for i in range(len(aug_text)):
                    text.append(aug_text[i])
                    slot.append(aug_slot[i])
                    intent.append(aug_intent[i])
                    tp_intent.append(aug_tp_intent[i])
                print(f'現資料集:{len(text)}, 增強:{len(aug_text)}')
            else:
                print("無資料增強")
        else:#測試集和驗證集
            text, slot, intent = self.__read_file(file_path)
            
        if if_train_file:#如果是訓練集的話，將訓練集的text, slot, intent整理並放入alphabet
        ...
        self.__digit_word_data[data_name] = self.__word_alphabet.get_index(text)#text的index
        if if_train_file:#如果是訓練集的話
            self.__digit_slot_data[data_name] = self.__slot_alphabet.get_index(slot)#slot的index
            self.__digit_intent_data[data_name] = self.__intent_alphabet.get_index(intent, multi_intent=True)#intent的index
            self.__digit_tp_intent_data[data_name] = tp_intent #intent_tp 的 index

    @staticmethod
    def __read_tp_file(file_path):#這邊是用來處理訓練集的，這是我寫的。
    ...
    @staticmethod#將文字、槽標籤、意圖標籤讀進來。
    def __read_file(file_path):#這邊是用來處理驗證集和測試集的
        """ 讀取給定路徑的資料文件。

         :param file_path: 資料文件路徑。
         :return: 句子列表、槽列表和意圖列表。
        """
        ...
        with open(file_path, 'r', encoding="utf8") as fr:
            for line in fr.readlines():#讀取每一行
                ...
                if len(items) == 1:#找到意圖標籤 ex: atis_abbreviation#atis_airport#atis_city
                    ...
                    # 清空 buffer lists，因為要重新裝文字和槽標籤
                    text, slot = [], []

                elif len(items) == 2:#文字和槽標籤 #ex: define O
                    text.append(items[0].strip())
                    slot.append(items[1].strip())

        return texts, slots, intents

    #用於讀取给定路徑的資料文件，並將文件内容解析成句子列表、槽位列表和意圖列表。
    def batch_delivery(self, data_name, batch_size=None, is_digital=True, shuffle=True):
        if batch_size is None:
            batch_size = self.batch_size

        if is_digital:#訓練集拿文字的index
            text = self.__digit_word_data[data_name]
            slot = self.__digit_slot_data[data_name]
            intent = self.__digit_intent_data[data_name]
            tp_intent_data = self.__digit_tp_intent_data[data_name]#放我寫的tp_intent
            tp_index = self.__intent_alphabet.get_index('TP')#獲取tp的index
            
            for i, tp_intent in enumerate(tp_intent_data):#要將每一個對話中的單詞都配一個意圖標籤，其中包括tp標籤。
                #print(tp_intent)
                count = 0
                intent[i]=intent[i][0]
                for j in range(len(tp_intent)):
                    if tp_intent[j] == 'TP':
                        tp_intent[j] = tp_index
                        #if j + 1 <= len(tp_intent):#不超過最後一個
                        if j + 1 < len(tp_intent) and tp_intent[j + 1] != 'TP':  # 添加索引檢查
                            count += 1
                    else:
                        if isinstance(intent[i], list) and len(intent[i]) > 1:  #如果current_intent是列表且長度大於 1，則考慮多意圖情况
                            tp_intent_data[i][j] = intent[i][count]
                        else:  #單意圖
                            tp_intent_data[i][j] = intent[i]
                #print(tp_intent)
                #input()
            intent = tp_intent_data
    ...
    #用于在文本序列中進行填充操作，以確保所有文本序列具有相同的長度。
    @staticmethod
