...
if __name__ == "__main__":
    ...
    MIXATIS, MIXSNIPS = ("MixATIS_TP", "MixSNIPS_TP")    # Domains
    for domain in [MIXATIS, MIXSNIPS]:
        for combination in all_combinations:#全部組合
            dataset = domain
            file_path = os.path.join("data", dataset, 'train.txt')#讀檔位置    

            texts, slot_tmp, intents, tp_intent = read_tp_file(file_path)#讀資料
            slots = [[] for _ in range(len(texts))]#將slot和tp合再一起處理。
            for i in range(len(texts)):
                for slot, tps in zip(slot_tmp[i], tp_intent[i]):
                    slots[i].append((slot, tps))
            ...
            for i, method in enumerate(methods):
                ...
                if method_num > 0:
                    if method == 'sr': #同義詞替換
                        ...
                        new_text, new_slots, new_intents = slot_sub_augment(texts, slots, slot_tmp, intents, method_num)
