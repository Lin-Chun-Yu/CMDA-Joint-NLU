...
class ModelManager(nn.Module):#從這裡建構模型的，從train.py所使用。
    ...
    def get_major_intent(self, sub_utterance_intent): #決定子對話的意圖標籤
        if len(sub_utterance_intent) == 0:#如果這個子對話沒有區塊意圖
            return None
        intents, counts = torch.unique(sub_utterance_intent, return_counts=True) # 找出唯一的意圖及其出現次數
        max_count = torch.max(counts).item()  # 找出最大出現次數
        major_intents = intents[counts == max_count]  # 找出所有出現次數為 max_count 的意圖
        if len(major_intents) == 1: # 如果只有一個意圖的出現次數最多
    ...
    def forward(self, text, seq_lens, window_seq_lens, window_size= 1, n_predicts=None, forced_slot=None, forced_intent=None):#在process.py會用到 n_predicts, forced_slot是slot的向量
        ...
        for j in range(window_num):
            temp = win_h[:, j:j + window_size, :]  # 切片獲取子張量，同時保留 batch_size 維度
            At = self.__WindowSelfattention(temp, window_size)  # self-attention At=(3, 384)         
            chunk_h[:, j] = torch.sum(At, dim=1)  # 在維度1上求和，即對視窗内的不同位置求和            
        ...
        for i in range(batch_size):#batch_size = 16 
            intent_window = pred_intent_window[i, 0:(window_seq_lens[i])]#第i的對話的chunk數量
            #print(intent_window)
            tp_indices = [j for j in range(window_seq_lens[i]) if intent_window[j] == TP_index]#得到第i的對話的chunk的tp的index
            current_utterance_intents = []
            
            prev_tp_idx = 0  # 前一個tp位置
            for tp_idx in tp_indices:#用tp進行切割
                sub_utterance_intent = intent_window[prev_tp_idx:tp_idx]#每一個子對話
                major_intent = self.get_major_intent(sub_utterance_intent) #決定這個子對話的意圖
                if major_intent is not None: #判斷是否為有沒有意圖
                    current_utterance_intents.append([i, major_intent]) #存起來第i對話的意圖
                prev_tp_idx = tp_idx + 1 #找下一個子對話
            ...
            unique_utterance_intents = list(map(list, set(map(tuple, current_utterance_intents))))  #因為可能會在不同子對話得到相同意圖，避免有重複的多意圖標籤
            all_sub_utterance_intents.extend(unique_utterance_intents)   #將這個對話的多意圖存起來
