...
class ModelManager(nn.Module):#從這裡建構模型的，從train.py所使用。
    ...
    def forward(self, text, seq_lens, window_seq_lens, window_size= 1, n_predicts=None, forced_slot=None, forced_intent=None):#在process.py會用到 n_predicts, forced_slot是slot的向量
        ...
        intent_lstm_out = self.__intent_lstm(g_hiddens, seq_lens) #hiddens (16, 42, 384)
        #intent_lstm_out = F.dropout(intent_lstm_out, p=self.__args.dropout_rate, training=self.training)
        #################################################我的作法
        #'''
        win_h = intent_lstm_out     
        batch_size, word_num, dimension = win_h.size()
        window_num = word_num - window_size + 1#chunk數量
        if window_num < 1:#遇到字數太少會無法切成chunk
            window_num = 1
        chunk_h = torch.zeros(batch_size, window_num, dimension)
        #print(seq_lens)

        seq_lens_tensor = torch.tensor(window_seq_lens)
        if self.__args.gpu:
            chunk_h = chunk_h.cuda()
        ...
        intent_index_sum = torch.cat(#閾值處理  intent_index_sum=[16, 18]
            [
                torch.sum(torch.sigmoid(pred_intent[i, 0:(window_seq_lens[i]), :]) > self.__args.threshold, dim=0).unsqueeze(0)
        ...
        if n_predicts is None:
            #pred_intent = torch.cat([pred_intent[i][:seq_lens[i]-window_size+1] for i in range(0, len(seq_lens))], dim=0)#處理intent_golden_labl。
            return F.log_softmax(pred_slot, dim=1), pred_intent #agif
