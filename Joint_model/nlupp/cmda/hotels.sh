...
datasets=(hotels) #banking hotels
        ...
        for percent in "${percents[@]}"; do #百分比
            ...
            while [ $ws -le 5 ]; do
                ...
                while [ $run -le 9 ]; do #fold0 ~ 9
                    ...
                    python train.py -g -bs=4 -ne=100 -dd=$dataset_dir -lod=$log_dir -sd=$save_dir -ws=$ws -ln=$line -afp=$aug_dir
                    run=`expr $run + 1`
                done
                ws=`expr $ws + 1`
            done    
...
#資料擴增
#python train.py -g -bs=4 -ne=100 -dd=./data/hotels0 -lod=./log/hotels/1 -sd=./save/hotels -ws=3 -ln=sr0.1ri0.1rs0.1rd0.1cr0.1ro0.1sl0.4.txt -afp=./data_aug/hotels0/1/sr0.1ri0.1rs0.1rd0.1cr0.1ro0.1sl0.4.txt
